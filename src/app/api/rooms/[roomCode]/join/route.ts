...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const [existing] = await db
      ...
      .from(players)
      .where(
        and(eq(players.roomCode, roomCode), eq(players.deviceId, deviceId)),
      )
      .limit(1);
    if (existing)
      return Response.json(
        { roomCode, playerId: existing.id },
    ...
    const limit = ["tic_tac_toe", "rps", "connect_four"].includes(room.gameType)
      ? 2
      : 4;
    if (roster.length >= limit)
      return Response.json(
        { error: `This room already has ${limit} players` },
        { status: 409, headers: privacyHeaders() },
      );
