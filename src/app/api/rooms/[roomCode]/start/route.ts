  ...
  try {
    ...
    if (room.gameType === "ludo") {
      if (roster.length < 2 || roster.length > 4) {
        return Response.json(
          { error: `Ludo requires 2 to 4 players. Current: ${roster.length}` },
          { status: 409, headers: privacyHeaders() },
        );
      }
    } else {
      const limit = ["tic_tac_toe", "rps", "connect_four"].includes(
        room.gameType,
      )
        ? 2
        : 4;
      if (roster.length !== limit)
        return Response.json(
          {
            error: `Exactly ${limit} players are needed. ${roster.length}/${limit} joined.`,
          },
          { status: 409, headers: privacyHeaders() },
        );
    }
    ...
    if (room.gameType === "chor_sipahi") {
      ...
      await db.transaction(async (tx: any) => {
        ...
        await tx
          .update(gameSessions)
          .set({ gameStatus: "playing", phase: "role_reveal", currentRound: 1 })
          .where(
            and(
              eq(gameSessions.roomCode, roomCode),
              eq(gameSessions.gameStatus, "waiting"),
            ),
          );
      });
    } else {
      ...
      if (room.gameType === "tic_tac_toe") initialState = initTicTacToe(roster);
        ...
        initialState = initConnectFour(roster);
