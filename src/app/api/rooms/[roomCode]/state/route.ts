...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const requester = requesterId
      ? roster.find(
          (player) =>
            player.id === requesterId && player.deviceId === requesterDevice,
        )
      : undefined;
    ...
    if (room.gameType !== "chor_sipahi") {
      baseResponse.gameState = room.gameState
        ...
        ? await db
            ...
            .where(
              and(
                ...
                eq(gameRounds.roundNumber, room.currentRound),
              ),
            )
            .limit(1)
        : [];
    ...
    const ownAssignment = requester
      ? assignments.find((assignment) => assignment.playerId === requester.id)
      : undefined;
    const publicRoles = publicRolesForPhase(
      room.phase,
    ...
    return Response.json(
      {
        ...
        rolesSeenCount: assignments.filter(
          (assignment) => assignment.hasSeenRole,
        ).length,
        publicRoles,
        result:
          round && (room.phase === "round_result" || room.phase === "game_over")
            ? {
                mantriGuessPlayerId: round.mantriGuessPlayerId,
    ...
    return Response.json(
      { error: "Could not load room state" },
