...
async function getCurrentRound(roomCode: string, roundNumber: number) {
  const [round] = await db
    ...
    .where(
      and(
        eq(gameRounds.roomCode, roomCode),
  ...
  try {
    ...
    if (!action.playerId)
      return Response.json(
        { error: "Missing player credentials" },
    ...
    if (!room)
      return Response.json(
        ...
        { status: 404, headers: privacyHeaders() },
      );
    const [actor] = await db
      ...
      .where(
        and(
          ...
          eq(players.roomCode, roomCode),
          eq(players.deviceId, deviceId),
        ),
      )
      .limit(1);
    ...
    if (!actor)
      return Response.json(
        { error: "Your guest session is not valid for this room" },
        { status: 403, headers: privacyHeaders() },
      );

    const roster = (await db
      .select()
      .from(players)
      .where(eq(players.roomCode, roomCode))
      .orderBy(asc(players.joinedAt))) as any[];
    ...
    if (!currentRound)
      return Response.json(
        ...
        { status: 409, headers: privacyHeaders() },
      );
    const assignments = (await db
      .select()
      .from(roundPlayers)
      .where(eq(roundPlayers.roundId, currentRound.id))) as any[];
    const actorAssignment = assignments.find(
      (assignment) => assignment.playerId === actor.id,
    );

    if (action.type === "acknowledge_role") {
      if (room.phase !== "role_reveal" || !actorAssignment)
        return Response.json(
          { error: "Role confirmation is not available now" },
          { status: 409, headers: privacyHeaders() },
        );
      await db
        .update(roundPlayers)
        ...
        .where(
          and(
            eq(roundPlayers.roundId, currentRound.id),
            eq(roundPlayers.playerId, actor.id),
          ),
        );
      const confirmed = await db
        ...
        .where(
          and(
            eq(roundPlayers.roundId, currentRound.id),
            eq(roundPlayers.hasSeenRole, true),
          ),
        );
      if (confirmed.length === 4)
        await db
          .update(gameSessions)
          .set({ phase: "minister_reveal" })
          .where(eq(gameSessions.roomCode, roomCode));
      return Response.json({ ok: true }, { headers: privacyHeaders() });
    }
    ...
    if (action.type === "reveal_mantri") {
      if (
        room.phase !== "minister_reveal" ||
        actorAssignment?.role !== "mantri"
      )
        return Response.json(
          { error: "Only the Mantri can reveal at this moment" },
          { status: 403, headers: privacyHeaders() },
        );
      await db
        .update(gameSessions)
    ...
    if (action.type === "guess_chor") {
      if (room.phase !== "guess" || actorAssignment?.role !== "mantri")
        return Response.json(
          { error: "Only the Mantri can make this guess" },
          { status: 403, headers: privacyHeaders() },
        );
      const suspectPlayerId = action.suspectPlayerId as string;
      const suspect = assignments.find(
        (assignment) => assignment.playerId === suspectPlayerId,
      );
      if (!suspect || !["chor", "sipahi"].includes(suspect.role))
        return Response.json(
          { error: "Choose one of the two unknown players" },
          { status: 400, headers: privacyHeaders() },
        );
      const result = calculateAwards(
      ...
      await db.transaction(async (tx: any) => {
        await Promise.all(
          assignments.map((assignment) =>
            tx
              .update(roundPlayers)
              .set({ pointsAwarded: result.awards[assignment.playerId] ?? 0 })
              .where(eq(roundPlayers.id, assignment.id)),
          ),
        );
        await Promise.all(
          assignments.map((assignment) =>
            tx
              .update(players)
              .set({
                score: sql`${players.score} + ${result.awards[assignment.playerId] ?? 0}`,
              })
        ...
        await tx
          .update(gameRounds)
          .set({
            mantriGuessPlayerId: suspectPlayerId,
            isGuessCorrect: result.correct,
            finishedAt: new Date(),
          })
        ...
        await tx
          .update(gameSessions)
          .set({
            phase: finalRound ? "game_over" : "round_result",
            gameStatus: finalRound ? "finished" : "playing",
            endedAt: finalRound ? new Date() : null,
          })
          .where(eq(gameSessions.roomCode, roomCode));
      });
    ...
    if (action.type === "next_round") {
      if (!actor.isHost || room.phase !== "round_result")
        return Response.json(
          { error: "Only the host can begin the next round" },
          { status: 403, headers: privacyHeaders() },
        );
      const roster = (await db
        .select()
        .from(players)
        .where(eq(players.roomCode, roomCode))) as any[];

      const nextRoundNumber = room.currentRound + 1;
      if (nextRoundNumber > room.roundsToPlay || roster.length !== 4)
        return Response.json(
          { error: "Cannot start another round" },
          { status: 409, headers: privacyHeaders() },
        );
      ...
      await db.transaction(async (tx: any) => {
        const [round] = await tx
          .insert(gameRounds)
          .values({ roomCode, roundNumber: nextRoundNumber })
          .returning();
        await tx
          .insert(roundPlayers)
          .values(
        ...
        await tx
          .update(gameSessions)
          .set({ currentRound: nextRoundNumber, phase: "role_reveal" })
          .where(eq(gameSessions.roomCode, roomCode));
      });
      return Response.json({ ok: true }, { headers: privacyHeaders() });
    }
