...
export default function Chess({ onBack }: { onBack: () => void }) {
  ...
  function click(r: number, c: number) {
    if (selected) {
      ...
      if (piece && piece.color === turn) {
        ...
        setTurn((t) => (t === "w" ? "b" : "w"));
        setSelected(null);
        setMoves((m) => [`${FILES[sc]}${8 - sr} → ${FILES[c]}${8 - r}`, ...m].slice(0, 16));
        return;
      }
    }
  ...
  return (
    <GameShell
      ...
      onBack={onBack}
      badge="STRATEGY"
      right={<button onClick={reset} className="heritage-button !min-h-10"><RotateCcw size={14} /> New Game</button>}
      sidebar={
        <Panel title="Moves">
          <div className="text-xs font-bold mb-3">{turn === "w" ? "White" : "Black"} to move</div>
          <div className="space-y-1 max-h-[280px] overflow-auto scrollbar-thin">
            {moves.length === 0 && <div className="text-xs text-[var(--ink-soft)] py-4 text-center">No moves yet</div>}
            {moves.map((m, i) => <div key={i} className="text-xs border border-[var(--ink)] bg-[var(--paper)] px-2 py-1">{i + 1}. {m}</div>)}
          </div>
          <div className="mt-3 text-xs text-[var(--ink-soft)]">
            Captured: {captured.slice(-8).map((p, i) => (
              <span key={i}>{UNI[p.color === "w" ? p.type.toUpperCase() : p.type]}</span>
            ))}
          </div>
        </Panel>
      }
      tips={["Click a piece, then click a destination square.", "White moves first.", "This local version focuses on clean UX and readable board state."]}
    >
            ...
            <div key={r} className="flex">
              {row.map((piece, c) => {
                ...
                return (
                  ...
                  >
                    {piece && (
                      <span className={piece.color === "w" ? "text-white drop-shadow" : "text-black"}>
                        {UNI[piece.color === "w" ? piece.type.toUpperCase() : piece.type]}
                      </span>
                    )}
                  </button>
                );
