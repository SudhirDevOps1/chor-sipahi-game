    ...
    <GameShell
      ...
      right={
        <div className="flex gap-2">
          <div className="border-2 border-[var(--ink)] bg-[var(--paper)] px-3 py-2 text-xs font-black">
            Bank {chips}
          </div>
          <button
            onClick={reset}
      ...
      sidebar={
        <Panel title="Round">
          ...
          <button
            onClick={roll}
            disabled={!totalBet || rolling}
            className="heritage-button w-full mt-4"
          >
      ...
      tips={[
        "Bet any amount on one or more symbols.",
        "Six dice are rolled together.",
        "If a symbol appears N times, that bet pays N×.",
      ]}
    >
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {SYMBOLS.map((s) => (
          <div
            key={s.id}
            className={`heritage-card p-4 ${bets[s.id] > 0 ? "outline outline-2 outline-[var(--saffron)]" : ""}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="serif text-xl">{s.label}</div>
                <div className="text-xs text-[var(--ink-soft)] font-bold">
                  Mark {s.mark}
                </div>
              </div>
      ...
      {result && (
        <div className="heritage-card p-5 mt-4">
          <div className="text-[10px] font-black tracking-widest uppercase text-[var(--ink-soft)] mb-3">
            Result
          </div>
          <div className="flex flex-wrap gap-2 mb-4">
            {result.dice.map((d, i) => (
              <div
                key={i}
                className="w-12 h-12 border-2 border-[var(--ink)] bg-[var(--paper)] grid place-items-center font-black"
              >
                {SYMBOLS[d].mark}
              </div>
            ))}
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-xs font-bold">
            {SYMBOLS.map((s) => (
              <div
                key={s.id}
                className={`border border-[var(--ink)] p-2 text-center ${result.counts[s.id] ? "bg-[#d8f3dc]" : "bg-[var(--paper)]"}`}
              >
