...
export function MultiGamePanel({
  state,
  playerId,
  busy,
  onAction,
  ...
  useEffect(() => {
    if (state.status === "finished") playWinSound();
  }, [state.status]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (gameState?.currentPlayerId) playMoveSound();
  ...
  if (state.status === "waiting") {
    ...
    const minP = isLudo
      ? 2
      : ["tic_tac_toe", "rps", "connect_four"].includes(gameType)
        ? 2
        : 4;
    const canStart = isLudo
      ? state.players.length >= 2 && state.players.length <= 4
      : state.players.length === minP;
    ...
    return (
      ...
      >
        ...
        >
          {gameLabels[gameType] || "Game"}
        </h1>
        <p style={{ color: "#94a3b8", fontSize: "0.9rem" }}>
          {isLudo
            ? "2 to 4 players can join"
            : `Exactly ${minP} players needed`}
        </p>
          ...
          {state.players.map((p, i) => (
            <div
              ...
              style={{
                ...
                minWidth: "110px",
                boxShadow:
                  p.id === playerId ? "0 0 20px rgba(129,140,248,0.3)" : "none",
              }}
            >
        ...
        {isHost ? (
          <button
            ...
            style={{
              ...
              boxShadow: canStart ? "0 6px 20px rgba(99,102,241,0.4)" : "none",
              transition: "all 0.3s",
            }}
          >
            Start Game 🚀
  ...
  if (state.status === "finished") {
    const winnerName =
      state.players.find((p) => p.id === gameState?.winnerId)?.name ?? "Draw";
    return (
      <div
        className="phase-card"
        style={{
  ...
  if (gameType === "tic_tac_toe") {
    ...
    return (
      ...
      >
        ...
        >
          2026 Edition - Multiplayer Ready
        </div>
        ...
        >
          <div style={{ textAlign: "center" }}>
            ...
            >
              Player X ({state.players[0]?.name || "P1"})
            </div>
            <div
              style={{
                fontSize: "1.8rem",
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div
              style={{ fontSize: "1.8rem", fontWeight: "bold", color: "#888" }}
            >
              {draws}
            </div>
          </div>
          <div style={{ textAlign: "center" }}>
            ...
            >
              Player O ({state.players[1]?.name || "P2"})
            </div>
            <div
              style={{
                fontSize: "1.8rem",
        ...
        >
          {gameState.winnerId
            ? `🎉 Player ${gameState.winnerId === state.players[0]?.id ? "X" : "O"} Wins!`
            : `Player ${gameState.currentPlayerId === state.players[0]?.id ? "X" : "O"}'s Turn`}
        </div>
          ...
          {gameState.board.map((cell: string | null, idx: number) => {
            ...
            return (
              <button
                key={idx}
                type="button"
                disabled={busy || !isMyTurn || isTaken}
                onClick={() =>
                  handleAction({ type: "make_move", cellIndex: idx })
                }
                style={{
                  aspectRatio: "1",
                  background: cellBg,
  ...
  if (gameType === "connect_four") {
    ...
    return (
      ...
      >
        ...
        >
          4 in a row to win • 2026 Edition
        </div>
            ...
            >
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  background:
                    "radial-gradient(circle at 30% 30%, #ffe066, #f59f00)",
                }}
              ></span>
        ...
        >
          {gameState.winnerId
            ? `🎉 ${gameState.winnerId === state.players[0]?.id ? "🔴 Red" : "🟡 Yellow"} Wins!`
            : `${gameState.currentPlayerId === state.players[0]?.id ? "🔴 Red" : "🟡 Yellow"}'s Turn`}
        </div>
              ...
              row.map((cell: string | null, cIdx: number) => {
                ...
                return (
                  <div
                    key={index}
                    onClick={() =>
                      handleAction({ type: "make_move", colIndex: cIdx })
                    }
                    style={{
                      background: cellBg,
                      borderRadius: "50%",
                      cursor: busy || !isMyTurn ? "default" : "pointer",
  ...
  if (gameType === "rps") {
    ...
    const opponentChoice = p1
      ? gameState.player2Choice
      : gameState.player1Choice;
    const oppIcon =
      showResults && opponentChoice ? icons[opponentChoice] : "❓";
      ...
      >
        ...
        >
          <div style={{ textAlign: "center" }}>
            ...
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#fa709a" }}
            >
              {p1 ? p1Score : p2Score}
            </div>
          </div>
          <div style={{ textAlign: "center" }}>
            ...
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#999" }}
            >
              {draws}
            </div>
          </div>
        ...
        >
          <div style={{ textAlign: "center" }}>
            ...
            <div
              className={`fighter-icon ${diceAnimating ? "shake" : ""}`}
              style={{
                fontSize: "4rem",
                minHeight: "80px",
                display: "flex",
          ...
          <div
            style={{ fontSize: "1.5rem", fontWeight: "bold", color: "#fa709a" }}
          >
            VS
          </div>
          <div style={{ textAlign: "center" }}>
        ...
        {!hasChosen && (
          ...
          >
            <button
              ...
              onClick={() =>
                handleAction({ type: "make_choice", choice: "rock" })
              }
              disabled={busy}
              style={{
                background: "#fff",
            ...
            >
              <span>✊</span>
              <span
                style={{ fontSize: "0.85rem", fontWeight: 600, color: "#666" }}
              >
                Rock
              </span>
            </button>
            <button
              className="choice-btn"
              onClick={() =>
                handleAction({ type: "make_choice", choice: "paper" })
              }
              disabled={busy}
              style={{
                background: "#fff",
            ...
            <button
              className="choice-btn"
              onClick={() =>
                handleAction({ type: "make_choice", choice: "scissors" })
              }
              disabled={busy}
              style={{
                background: "#fff",
  ...
  if (gameType === "ludo") {
    ...
    const activeColor =
      colors[
        state.players.findIndex((p) => p.id === gameState.currentPlayerId) % 4
      ] || "red";
      ...
      >
        ...
        >
          ...
          >
            ...
            />
            <span>
              {activeColor.charAt(0).toUpperCase() + activeColor.slice(1)}'s
              Turn
            </span>
          </div>
          ...
          {Array.from({ length: 15 }).map((_, r) => {
            ...
            return Array.from({ length: 15 }).map((_, c) => {
              ...
              if (row < 6 && col < 6) bg = "#ff6b6b";
                ...
                if (row === 7 && col === 7)
                  bg =
                    "conic-gradient(#ff6b6b 0 25%, #51cf66 25% 50%, #ffd43b 50% 75%, #4dabf7 75% 100%)";
                else bg = "#f1f3f5";
              } else {
                // Colored Paths
                ...
                if (isHomeRed) bg = "#ffc9c9";
                  ...
                  bg = "#d3f9d8"; // Green start
                else if (row === 8 && col === 13)
                  bg = "#fff3bf"; // Yellow start
                  ...
                  if (stepPos >= 0 && stepPos <= 30) {
                    ...
                    if (stepPos >= 0 && stepPos < 24) {
                      const perimeterIndex =
                        (LUDO_START_POS[pIdx % 4] + stepPos) % 52;
                    ...
                    if (
                      ...
                      const isMovable =
                        ...
                        p.id === playerId &&
                        !(
                          (stepPos === -1 && gameState.diceRoll !== 6) ||
                          stepPos + (gameState.diceRoll || 0) > 30
                        );
                      matchingTokens.push({
                        playerIndex: pIdx,
                        tokenIndex: tIdx,
                      ...
                      style={{
                        ...
                        background:
                          t.color === "red"
                            ...
                            : t.color === "green"
                              ? "radial-gradient(circle at 30% 30%, #8ce99a, #2b8a3e)"
                              : t.color === "yellow"
                                ? "radial-gradient(circle at 30% 30%, #ffe066, #e67700)"
                                : "radial-gradient(circle at 30% 30%, #74c0fc, #1864ab)",
                        color: t.color === "yellow" ? "#333" : "#fff",
                        fontSize: "0.55rem",
                        fontWeight: "bold",
                        cursor: t.isMovable ? "pointer" : "default",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        zIndex: 10,
                        transform: t.isMovable ? "scale(1.15)" : "none",
                      }}
                    >
                      {t.tokenIndex + 1}
                    </button>
