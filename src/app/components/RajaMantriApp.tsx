...
export function RajaMantriApp() {
  ...
  async function createRoom(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const result = await request<RoomResponse>("/api/rooms", { method: "POST", headers: headers(true), body: JSON.stringify({ playerName: createName, roundsToPlay: rounds, isPrivate: true }) });
...
function Landing({ createName, setCreateName, joinName, setJoinName, joinCode, setJoinCode, rounds, setRounds, rooms, busy, onCreate, onJoin, onRoomPick, onRefresh }: { createName: string; setCreateName: (value: string) => void; joinName: string; setJoinName: (value: string) => void; joinCode: string; setJoinCode: (value: string) => void; rounds: 3 | 5 | 10; setRounds: (value: 3 | 5 | 10) => void; rooms: RoomSummary[]; busy: boolean; onCreate: (event: React.FormEvent) => void; onJoin: (event: React.FormEvent) => void; onRoomPick: (code: string) => void; onRefresh: () => void }) {
  return <><main className="landing"><section className="intro"><p className="eyebrow"><Sparkles size={14} /> A SCHOOLYARD CLASSIC, ONLINE</p><h1>Guess the thief.<br /><em>Guard your crown.</em></h1><p className="intro-copy">Raja Mantri Chor Sipahi is the beloved four-player bluffing game. Deal secret roles, read your friends, and make one decisive guess.</p><div className="role-stamps"><span>👑 Raja <b>1000</b></span><span>🗡️ Mantri <b>800</b></span><span>💰 Chor <b>?</b></span><span>🛡️ Sipahi <b>500</b></span></div></section><section className="paper-stack"><div className="paper-note paper-back" /><div className="paper-note paper-main"><p className="eyebrow">ONE TABLE · FOUR FRIENDS</p><h3>Ready for a round?</h3><div className="paper-roles"><span>👑</span><span>🗡️</span><span>💰</span><span>🛡️</span></div><p>Private role cards. No downloads. No account.</p></div></section></main><section className="lobby-area"><div className="lobby-card"><div className="lobby-card-heading"><span className="card-number">01</span><div><p className="eyebrow">OPEN A TABLE</p><h2>Create a private room</h2></div></div><form onSubmit={onCreate} className="lobby-form"><label>Your name<input value={createName} onChange={(event) => setCreateName(event.target.value)} maxLength={24} placeholder="e.g. Aanya" required /></label><label>How many rounds?<select value={rounds} onChange={(event) => setRounds(Number(event.target.value) as 3 | 5 | 10)}><option value={3}>3 quick rounds</option><option value={5}>5 classic rounds</option><option value={10}>10 long-game rounds</option></select></label><button className="heritage-button" disabled={busy}>Create private room <ArrowRight size={17} /></button></form></div><div className="lobby-card join-card"><div className="lobby-card-heading"><span className="card-number">02</span><div><p className="eyebrow">HAVE A CODE?</p><h2>Join your friends</h2></div></div><form onSubmit={onJoin} className="lobby-form"><label>Room code<input value={joinCode} onChange={(event) => setJoinCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="6 digits" required /></label><label>Your name<input value={joinName} onChange={(event) => setJoinName(event.target.value)} maxLength={24} placeholder="e.g. Kabir" required /></label><button className="outline-button" disabled={busy || joinCode.length !== 6}>Join room <ChevronRight size={17} /></button></form></div><div className="open-rooms"><div className="open-rooms-head"><div><p className="eyebrow">OPTIONAL</p><h2>Open tables</h2></div><button type="button" onClick={onRefresh} aria-label="Refresh public rooms">↻</button></div>{rooms.length ? <div className="room-mini-list">{rooms.map((room) => <button type="button" key={room.roomCode} onClick={() => onRoomPick(room.roomCode)}><strong>{room.roomCode}</strong><span>{room.playersCount}/4 players · {room.roundsToPlay} rounds</span><ChevronRight size={15} /></button>)}</div> : <div className="empty-rooms"><Users size={22} /><strong>No public table is waiting</strong><span>Create a private room and share its code instead.</span></div>}</div></section></>;
}
...
function GameRoom({ state, playerId, busy, onAction, onStart, onInvite, onLeave, onChat, chatDraft, setChatDraft }: { state: RajaGameState; playerId: string; busy: boolean; onAction: (payload: Record<string, string>) => void; onStart: () => void; onInvite: () => void; onLeave: () => void; onChat: (event: React.FormEvent) => void; chatDraft: string; setChatDraft: (value: string) => void }) {
  ...
  const phaseCopy: Record<string, string> = { lobby: "Waiting for all four players", role_reveal: "Read your secret role", minister_reveal: "Raja is calling for the Mantri", guess: "Mantri must identify Chor", round_result: "The truth is on the table", game_over: "The crown has been decided" };
  return <main className="game-page"><div className="game-toolbar"><button type="button" className="leave-button" onClick={onLeave}><LogOut size={15} /> Leave</button><div className="room-code-toolbar"><span>PRIVATE ROOM</span><strong>{state.roomCode}</strong><button type="button" onClick={onInvite}><Copy size={14} /> Invite</button></div><span className="sync-pill"><span /> Live sync</span></div><div className="phase-banner"><span>ROUND {Math.max(state.currentRound, 1)} / {state.roundsToPlay}</span><strong>{phaseCopy[state.phase]}</strong><i /></div><div className="game-grid"><section className="table-panel"><PlayerTable players={state.players} playerId={playerId} publicRoles={state.publicRoles} /><PhasePanel state={state} playerId={playerId} candidates={candidates} isHost={isHost} busy={busy} onAction={onAction} onStart={onStart} /></section><aside className="game-side"><ScorePanel ranked={rank} playerId={playerId} /><ChatPanel messages={state.chat} draft={chatDraft} setDraft={setChatDraft} onSubmit={onChat} /></aside></div></main>;
}
