...
export const gamePhaseValues = ["lobby", "role_reveal", "minister_reveal", "guess", "round_result", "game_over"] as const;
...
export const gameSessions = sqliteTable("game_sessions", {
  ...
  gameStatus: text("game_status", { enum: gameStatusValues }).default("waiting").notNull(),
...
export const players = sqliteTable("players", {
  ...
  roomCode: text("room_code", { length: 6 }).references(() => gameSessions.roomCode, { onDelete: "cascade" }).notNull(),
  ...
  joinedAt: integer("joined_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now') * 1000)`).notNull(),
...
export const gameRounds = sqliteTable("game_rounds", {
  ...
  roomRoundIndex: uniqueIndex("game_rounds_room_round_idx").on(table.roomCode, table.roundNumber),
}));

export const roundPlayers = sqliteTable("round_players", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  roundId: integer("round_id").references(() => gameRounds.id, { onDelete: "cascade" }).notNull(),
  playerId: text("player_id", { length: 36 }).references(() => players.id, { onDelete: "cascade" }).notNull(),
  role: text("role", { enum: gameRoleValues }).notNull(),
  hasSeenRole: integer("has_seen_role", { mode: "boolean" }).default(false).notNull(),
  pointsAwarded: integer("points_awarded").default(0).notNull(),
}, (table) => ({
  roundPlayerIndex: uniqueIndex("round_players_round_player_idx").on(table.roundId, table.playerId),
  playerIndex: index("round_players_player_idx").on(table.playerId),
}));

export const movesHistory = sqliteTable("moves_history", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  roomCode: text("room_code", { length: 6 }).references(() => gameSessions.roomCode, { onDelete: "cascade" }).notNull(),
  ...
  roomTurnIndex: index("moves_room_turn_idx").on(table.roomCode, table.turnNumber),
}));
