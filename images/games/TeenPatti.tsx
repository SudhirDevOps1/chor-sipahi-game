...
function scoreHand(cards: Card[]) {
  ...
  if (s[0].rank === s[1].rank && s[1].rank === s[2].rank) return { rank: "Trail", score: 500 + s[0].rank };
  if (sameSuit && consec) return { rank: "Pure Sequence", score: 400 + s[0].rank };
  if (consec) return { rank: "Sequence", score: 300 + s[0].rank };
  if (sameSuit) return { rank: "Color", score: 200 + s[0].rank };
  if (s[0].rank === s[1].rank || s[1].rank === s[2].rank) return { rank: "Pair", score: 100 + s[1].rank };
...
export default function TeenPatti({ onBack }: { onBack: () => void }) {
  ...
  function deal() {
    const deck = shuffle();
    const next = Array.from({ length: players }, (_, i) => deck.slice(i * 3, i * 3 + 3));
    setHands(next);
  ...
  function revealAll() {
    ...
    const winners = ranks.map((r, i) => (r.score === best ? i : -1)).filter((i) => i >= 0);
    setMessage(`Winner: Player ${winners.map((i) => i + 1).join(", ")} · ${ranks[winners[0]].rank}`);
    setScores((s) => {
      const n = [...s];
    ...
    <GameShell
      ...
      sidebar={
        <Panel title="Round Controls">
          ...
          <button onClick={revealAll} className="heritage-button w-full mb-2"><Eye size={14} /> Show All</button>
          <button onClick={deal} className="heritage-button secondary w-full"><RotateCcw size={14} /> New Deal</button>
        </Panel>
      }
      tips={["Trail beats pure sequence, sequence, color, pair, then high card.", "Tap any player card stack to reveal it.", "Best hand wins the round."]}
    >
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {hands.map((cards, i) => {
          ...
          return (
            <button key={i} onClick={() => setRevealed((r) => { const n=[...r]; n[i]=true; return n; })} className="heritage-card p-4 text-left hover:-translate-y-0.5 transition">
              <div className="flex items-center justify-between mb-3">
                <div className="font-black">Player {i + 1}</div>
                <div className="text-xs font-bold text-[var(--ink-soft)]">Wins {scores[i]}</div>
              </div>
              <div className="flex gap-2">
                {cards.map((c, j) => (
                  <div key={j} className={`w-14 h-20 border-2 border-[var(--ink)] grid place-items-center font-black ${open ? "bg-white" : "bg-[var(--ink)] text-[var(--paper)]"} ${open && (c.suit === "♥" || c.suit === "♦") ? "text-red-600" : ""}`}>
                    {open ? `${DISP[c.rank] || c.rank}${c.suit}` : "?"}
                  </div>
                ))}
              </div>
              <div className="mt-3 text-xs font-bold text-[var(--ink-soft)]">{rank}</div>
            </button>
          );
        })}
      </div>
